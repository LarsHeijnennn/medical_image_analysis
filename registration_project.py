...
def intensity_based_registration(
    I,
    Im,
    method='rigid_cc',
    num_iter=200,
    ...
    if len(x) == 3:
        ...
        T = (
            reg.rotate(x[0])
            .dot(reg.scale(x[1], x[2]))
            .dot(reg.shear(x[3], x[4]))
        )
