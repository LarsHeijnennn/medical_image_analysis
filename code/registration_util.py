...
def my_cpselect(imagePath1, imagePath2):
	# Pops up a matplotlib window in which to select control points on the two images given as input.
	#
	# Input:
    # imagePath1 - fixed image path
	...
	#accumulate points
	points = plt.ginput(n=-1, timeout=300)
	plt.close(fig)
	...
	#subdivide the points into two different arrays. If the current number is 
 	# even belongs to the first image, and uneven to the second image. 
  	# (Assuming the points were entered in the correct order.)
	#X and Y values are on rows, with each column being a pair of values.
	k = len(points)//2
	X = np.empty((2,k))
	X[:] = np.nan
